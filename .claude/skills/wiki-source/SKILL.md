...
Follow the Ingest procedure in `wiki/Malan.wiki/SCHEMA_Malan.md`. Pointers:
...
7. Update `index_Malan.md` under the "Source summaries" category.
8. Append a `## [YYYY-MM-DD] ingest | Source title` entry to `log_Malan.md`. The first bullet is the attribution line `- by: <name> via claude-code`, where `<name>` is the output of `git config user.name` in the wiki repo (read it, do not invent it). Then 2 to 5 bullets describing the ingest. See "Log Entry Attribution" in `SCHEMA_Malan.md`.
