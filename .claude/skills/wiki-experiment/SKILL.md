...
Follow the Ingest procedure defined in `wiki/Malan.wiki/SCHEMA_Malan.md`. The steps below are pointers, not a substitute for SCHEMA:
...
7. Update `index_Malan.md` with one-line descriptions of new pages in the right category.
8. Append a `## [YYYY-MM-DD] update | Experiment name` entry to `log_Malan.md`. The first bullet is the attribution line `- by: <name> via claude-code`, where `<name>` is the output of `git config user.name` in the wiki repo (read it, do not invent it). Then 2 to 5 bullets describing the run. See "Log Entry Attribution" in `SCHEMA_Malan.md`.
